...
void markPoint(Vec3 points, Vec3 colors, float size)
{
    ...
    for (int i = 0; i < 5; ++i) {
        float angle1 = i * 2.0f * 3.1415926f / 5.0f;
        float angle2 = (i + 2) * 2.0f * 3.1415926f / 5.0f;
        glVertex3f(points.X, points.Y, points.Z);
        glVertex3f(points.X + size * cos(angle1), points.Y + size * sin(angle1), points.Z);
        glVertex3f(points.X + size * cos(angle2), points.Y + size * sin(angle2), points.Z);
    }
    glEnd();
...
void display()
{
    ...
    // gambar lintasan lemniskata (infinity)
    float size = 1.0f; // ukuran lemniskata
    float c = 0.8f; // bentuk dari lemniskata
    glColor3f(0.0f, 1.0f, 0.0f); // warna lemniskata
    glBegin(GL_LINE_STRIP);
    for (int i = 0; i <= 100; i++) {
        float theta = 2.0f * 3.1415926f * float(i) / 100.0f;
        float x = size * sqrt(2.0f) * cos(theta) / (sin(theta) * sin(theta) + 1);
        float y = size * sqrt(2.0f) * cos(theta) * sin(theta) / (sin(theta) * sin(theta) + 1);
        glVertex2f(x, y);
    }
    glEnd();

    // gambar bintang kecil mengelilingi lintasan lemniskata
    for (int i = 0; i < NUM_BOXES; i++) {
        float theta = 2.0f * 3.1415926f * float(i) / float(NUM_BOXES);
        float x = size * sqrt(2.0f) * cos(theta) / (sin(theta) * sin(theta) + 1);
        float y = size * sqrt(2.0f) * cos(theta) * sin(theta) / (sin(theta) * sin(theta) + 1);
        markPoint(Vec3(x, y, 0.0f), Vec3(1.0f, 0.0f, 0.0f), 0.05f); // warna merah untuk bintang kecil
    }
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("AULIA IRFAN HAKIM / 2200018329 - POST 7");
